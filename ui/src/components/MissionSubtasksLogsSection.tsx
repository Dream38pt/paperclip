...
export function MissionSubtasksLogsSection({ issueId, companyId }: MissionSubtasksLogsSectionProps) {
  ...
  return (
    <section className="rounded-lg border border-border bg-card">
      <button
        type="button"
        className="flex w-full flex-col items-stretch gap-2 px-3 py-2.5 text-left sm:flex-row sm:items-center sm:justify-between sm:gap-3"
      ...
      >
        ...
        {missingLogCount > 0 ? (
          <span className="inline-flex w-fit max-w-full shrink-0 items-center gap-1 rounded-full border border-destructive/40 bg-destructive/10 px-2 py-0.5 text-xs font-medium text-destructive">
            <AlertTriangle className="h-3 w-3" />
            <span className="truncate">Logs manquants {missingLogCount}</span>
          </span>
        ) : null}
      </button>
            ...
            <>
              <div className="space-y-2 md:hidden">
                {rows.map(({ subtask, signal, missingLog, documentsLoading, commentsLoading }) => (
                  <div key={subtask.id} className="rounded-lg border border-border/70 p-3">
                    <Link
                    ...
                    >
                      ...
                      <span className="block truncate text-sm font-medium text-foreground">{subtask.title}</span>
                    </Link>

                    <dl className="mt-3 grid grid-cols-[minmax(5.5rem,auto)_minmax(0,1fr)] gap-x-3 gap-y-2 text-xs">
                      <dt className="text-muted-foreground">Status</dt>
                      <dd className="min-w-0">
                        <span className="inline-flex max-w-full items-center gap-1.5">
                          <StatusIcon status={subtask.status} blockerAttention={subtask.blockerAttention} />
                          <span className="truncate capitalize text-muted-foreground">{subtask.status.replaceAll("_", " ")}</span>
                        </span>
                      </dd>
  ...
  if (signal.logDocuments.length > 0) {
    ...
    return (
      <span className="inline-flex max-w-full min-w-0 items-center gap-1">
        ...
        <span className="truncate">{document.title || document.key}</span>
      </span>
    );
  }
  if (signal.latestClosureComment) {
    return (
      <a href={`#comment-${signal.latestClosureComment.id}`} className="inline-flex max-w-full min-w-0 items-center gap-1 underline-offset-2 hover:underline">
        <MessageSquare className="h-3.5 w-3.5 shrink-0" />
        <span className="truncate">{relativeTime(signal.latestClosureComment.createdAt)}</span>
      </a>
    );
  }
